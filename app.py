import streamlit as st, pandas as pd, yfinance as yf, warnings, logging, os, io, contextlib
import plotly.graph_objects as go
warnings.filterwarnings("ignore"); logging.getLogger("yfinance").setLevel(logging.CRITICAL)
st.set_page_config(layout="wide"); st.title("NSE 227 - PART 1 (1-113) BOX + Candles")
...
st.success(f"Loaded {len(companies)} Part 1")
st.caption(f"Total 227, This App {len(companies)}, Other App {len(all_companies)-113}")
...
if len(live)==0:
    st.warning("Yahoo busy, Reboot in 2 mins"); st.stop()
...
if df is not None:
    fig=go.Figure()
    fig.add_trace(go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#00FF00', decreasing_line_color='#FF0000', increasing_fillcolor='#00FF00', decreasing_fillcolor='#FF0000'))
    fig.add_hline(y=row['Support'], line_dash="dash", line_color="green"); fig.add_hline(y=row['Resistance'], line_dash="dash", line_color="red")
